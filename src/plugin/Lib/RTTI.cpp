...
extern "C" char *__cdecl __unDName(char *outputString, const char *name, int maxStringLength, void *pAlloc, void *pFree,
...
bool RTTI::GetCompleteObjectLocator()
{
    ...
    if (!DbgMemRead((duint)m_vftable.pCompleteObjectLocator, &m_completeObjectLocator,
                    sizeof(RTTICompleteObjectLocator)))
    {
        return false;
    }
...
bool RTTI::GetBaseClasses()
{
    ...
    {
        dprintf("Found %d base classes in the ClassHierarchyDescriptor, this seems unlikely, maximum we can save is "
                "%d.  Aborting.\n",
...
std::string RTTI::GetNameOnly(duint addr)
{
    ...
    vftable_t vtbl = {};
    if (!DbgMemRead(vftable - sizeof(duint), &vtbl, sizeof(vftable_t)))
    {
        return "";
    }

    RTTICompleteObjectLocator col = {};
    if (!DbgMemRead((duint)vtbl.pCompleteObjectLocator, &col, sizeof(RTTICompleteObjectLocator)))
    {
        return "";
    }
    ...
    if (!DbgMemRead(pTypeDesc, &typeDesc, sizeof(TypeDescriptor)))
    {
        return "";
    }
